...
local state = {
    ...
    selectedFileIndex = 1,
    -- Image generation state (bridge-based, no threads)
    isGenerating = false,
    generationError = nil,
    -- Storybook UI state
...
function editor.update(dt)
    ...
    -- Image generation is now handled via bridge callbacks, no polling needed
end
...
function editor.startImageGeneration(page)
    ...
    -- Use bridge for image generation (works in both web and native mode)
    local params = {
        description = page.image_prompt or "",
        question = page.question_text or ""
    }
    print("[Editor] Requesting image via bridge:")
    print("[Editor]   Description: " .. params.description)
    print("[Editor]   Question: " .. params.question)

    bridge.requestImageGeneration(params, function(result, error)
        state.isGenerating = false
        if error then
            state.generationError = error
            editor.showMessage("Error: " .. error)
            print("[Editor] Image generation failed: " .. error)
        else
            local currentPage = state.story.pages[state.selectedPageIndex]
            if currentPage and result and result.imageUrl then
                currentPage.image_path = result.imageUrl
                editor.showMessage("Image generated!")
                print("[Editor] Image generated, path: " .. result.imageUrl)
            end
        end
    end)
...
function editor.startCoverGeneration()
    ...
    -- Use bridge for cover generation
    local params = {
    ...
    bridge.requestImageGeneration(params, function(result, error)
        ...
        if error then
            state.generationError = error
            editor.showMessage("Error: " .. error)
            print("[Editor] Cover generation failed: " .. error)
        else
            if result and result.imageUrl then
                state.story.cover_image_path = result.imageUrl
                state.storySettingsExpanded = false
                state.selectedPageIndex = 0
                editor.showMessage("Cover generated!")
                print("[Editor] Cover generated, path: " .. result.imageUrl)
            end
        end
    end)
